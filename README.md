...
- **[`usage_scores.py`](usage_scores.py)** - clones all 98 repos, scans source code and measures how heavily each dependency is actually used. Outputs `usage_scores.csv`.
- **[`originality.py`](originality.py)** - the main scoring model. Loads six CSV/JSON data sources, engineers features, runs a two-pass producer/consumer algorithm, classifies repos into categories, and computes a formula-based originality score tuned on 16 public jury anchors.
